...
describe('100% single-fund portfolio', () => {
  test('cumulative values track fund price exactly', () => {
    ...
    const { cumulativeValues } = computePortfolio(alloc, data, { rebalance: false, useTotalReturn: true })
  ...
  test('zero volatility for perfectly smooth monthly returns', () => {
    ...
    const { annualizedVolatility } = computePortfolio(alloc, data, { rebalance: false, useTotalReturn: true })
  ...
  test('zero max drawdown for monotonically rising prices', () => {
    ...
    const { maxDrawdown } = computePortfolio(alloc, data, { rebalance: false, useTotalReturn: true })
  ...
  test('CAGR matches analytic value for known price path', () => {
    ...
    const { cagr } = computePortfolio(alloc, data, { rebalance: false, useTotalReturn: true })
  ...
  test('Sharpe ratio uses correct formula', () => {
    ...
    const { cagr, annualizedVolatility, sharpeRatio } = computePortfolio(
      alloc, data, { rebalance: false, useTotalReturn: true }
    )
...
describe('rebalancing', () => {
  ...
  test('rebalance=true produces different annual returns than rebalance=false', () => {
    ...
    const withRebalance = computePortfolio(alloc, data, { rebalance: true, useTotalReturn: true })
    const withoutRebalance = computePortfolio(alloc, data, { rebalance: false, useTotalReturn: true })
...
describe('useTotalReturn toggle', () => {
  test('total-return uses adjusted_close, price-return uses close', () => {
    ...
    const totalReturn = computePortfolio(alloc, data, { rebalance: false, useTotalReturn: true })
    const priceReturn = computePortfolio(alloc, data, { rebalance: false, useTotalReturn: false })
...
describe('edge cases', () => {
  test('single month of data returns initial value with zero stats', () => {
    ...
    const result = computePortfolio(alloc, data, { rebalance: false, useTotalReturn: true })
  ...
  test('zero-weight funds are ignored and do not change results', () => {
    ...
    const without = computePortfolio(
      [{ ticker: 'A', weight: 100 }],
      data,
      { rebalance: false, useTotalReturn: true },
    )
  ...
  test('empty allocations returns empty result', () => {
    const result = computePortfolio([], {}, { rebalance: false, useTotalReturn: true })
...
describe('performance', () => {
  test('4 funds × 180 months completes in under 16ms (average of 200 runs)', () => {
    ...
    const config = { rebalance: true, useTotalReturn: true }
